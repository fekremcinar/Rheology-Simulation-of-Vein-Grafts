...
==============================================================
Pipeline for experiment 02_heartbeat_sinusoidal (pulsatile Poiseuille flow):
  ...
  7. Inlet-pressure chart — p_inlet_sim vs p_outlet_sim vs p_inlet_prescribed [mmHg]
...
Option A — pvpython (command line, outside ParaView):
    pvpython ~/Rheology-Simulation-of-Vein-Grafts/assets/paraview/02_heartbeat_sinusoidal.py \\
        --case ~/Rheology-Simulation-of-Vein-Grafts/run/02_heartbeat_sinusoidal \\
        --radius 0.005
...
print(f"\n[02_heartbeat_sinusoidal] Loading: {FOAM_FILE}")
...
print(f"[02_heartbeat_sinusoidal]   Vessel radius  : {RADIUS} m")
print(f"[02_heartbeat_sinusoidal]   Inlet profile  : x = {x_inlet:.4f} m  ({inlet_x_mm} mm)")
print(f"[02_heartbeat_sinusoidal]   Outlet profile : x = {x_outlet:.4f} m  ({outlet_x_mm} mm)")
...
def _style_series(disp, styles):
    ...
    try:
        ...
        print(f'[02_heartbeat_sinusoidal] Note: series styling skipped ({e})')
...
try:
    ...
    # Analytical Q(t) = π R²/2 · U_center(t)
    q_ana = [
        math.pi * RADIUS**2 / 2
        * (U_AVG + U_AMP * math.sin(2 * math.pi / T_PERIOD * t - math.pi / 2))
        * M3_TO_ML
        for t in times
    ]
    ...
    _style_series(disp_q, [
        ...
        {'name': 'Q_analytical', 'color': (0.2,   0.2,   0.2),   'line': 2, 'thickness': 1},
    ])
...
#       p_outlet = −0.525 · p₀ + 1.525 · p₁  (extrapolate to x = L, should ≈ 0)
#
# Units: kinematic p → Pa  (×ρ),  Pa → mmHg  (÷133.322).
# Analytical overlay: prescribed inlet BC  p_inlet = ρ · 4νL/R² · U_center(t).

PA_TO_MMHG = 1.0 / 133.322   # 1 Pa = 0.007501 mmHg
MAP_MMHG   = 100.0            # mean arterial pressure [mmHg]  = (120+80)/2
A_ABS_MMHG =  20.0            # pulse amplitude [mmHg]         = (120-80)/2
...
try:
    ...
    # Prescribed analytical inlet pressure [mmHg]:
    #   p_inlet = MAP + A_abs·sin(ωt−π/2) + ρ·4νL/R²·U_center(t)·PA_TO_MMHG
    _coeff_local = RHO * 4.0 * NU * VESSEL_L / RADIUS**2 * PA_TO_MMHG
    _p_ana_mmhg = [
        MAP_MMHG
        + A_ABS_MMHG * math.sin(2 * math.pi / T_PERIOD * t - math.pi / 2)
        + _coeff_local * (U_AVG + U_AMP * math.sin(2 * math.pi / T_PERIOD * t - math.pi / 2))
        for t in _ts_p
    ]
    ...
    _tbl_p = _make_table_source([
        ...
        ('p_inlet_prescribed', _p_ana_mmhg),
    ])
    ...
    _style_series(_disp_p, [
        ...
        {'name': 'p_inlet_prescribed', 'color': (0.2,   0.2,   0.2),   'line': 2, 'thickness': 1},
    ])
...
print('  Flow-rate chart  : blue=inlet  orange=outlet  grey-dashed=analytical')
print('  Pressure chart   : blue=p_inlet_sim  red=p_outlet_sim  grey-dashed=prescribed  [mmHg]')
