...
Outputs:
  ...
  assets/scripts/data/qt_biphasic.npy   -- shape (200, 3): (t, Q_m3s, U_mean_ms)
  assets/scripts/data/qt_triphasic.npy  -- shape (200, 3): (t, Q_m3s, U_mean_ms)
...
# Save numpy arrays (shape: N x 3  -> columns: t, Q_m3s, U_mean_ms)
bip_arr = np.column_stack([bip_td, bip_Qd, bip_Ud])
tri_arr = np.column_stack([tri_td, tri_Qd, tri_Ud])
np.save(os.path.join(DATA_DIR, "qt_biphasic.npy"),  bip_arr)
np.save(os.path.join(DATA_DIR, "qt_triphasic.npy"), tri_arr)
print()
print(f"Saved: qt_biphasic.npy   shape={bip_arr.shape}  cols=(t, Q_m3s, U_mean_ms)")
print(f"Saved: qt_triphasic.npy  shape={tri_arr.shape}  cols=(t, Q_m3s, U_mean_ms)")
